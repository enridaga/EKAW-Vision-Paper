...
\begin{abstract}
Knowledge graphs have become the primary vehicle for data integration and are critical to the success of modern AI — but the diversity of KG modelling practices, from lightweight vocabularies to richly axiomatised ontologies, makes integration and reuse expensive and brittle. This challenge is particularly acute in neuro-symbolic AI, where bridging neural and symbolic components depends on the ability to reengineer KGs to fit new requirements; GenAI now offers unprecedented automation capability, but without a principled understanding of the KG space, such automation remains conceptually ungrounded. We introduce the \emph{ontological continuum} as that missing conceptualisation, a theoretical construct whose positions are characterised along two axes: semantics $\leftrightarrow$ pragmatics, and properties $\leftrightarrow$  affordances; together these define a vocabulary to describe, compare, navigate, and transform KGs across the full range of modelling practices. The methodological stance is empirical: rather than prescribing how KGs should be modelled, the continuum aims to define a theory of the existent, derived from observation of real-world KG engineering practices and whose structure can be made formally explicit through Formal Concept Analysis (FCA). We ground the vision through two case studies: one on provenance knowledge, showing how a single concern manifests differently across the continuum; and one on upper-level ontological commitment, showing how an early foundational choice cascades through the characterisation dimensions to determine downstream tooling and interoperability affordances. We articulate five open research challenges and invite the community to develop the ontological continuum as a shared research agenda.
\end{abstract}
...
\add{The contribution of this paper is conceptual and terminological. We define the meta-model of the continuum: a characterisation framework, organised around two axes, \emph{semantics versus pragmatics}, and \emph{properties versus affordances}, that provides a vocabulary for describing, comparing, navigating, and transforming KGs across the full range of modelling practices. We ground the vision through an empirical case study on provenance knowledge, demonstrating that KGs covering a single concern can occupy very different positions in the continuum and that the structure of those positions can be made explicit and formally ordered through Formal Concept Analysis. We articulate open research challenges and we discuss what these entail for the definition of the continuum.}
...
ad~hoc pragmatic adaptation that the field currently has no principled
framework to support~\cite{guizzardi2021,morlidge2026animlontology}.}
...
alignment systems (LogMap~\cite{logma_ecai2012}, AML~\cite{aml}), and
modularisation techniques enable integration across KGs but fall short
of addressing diversity in a unified way. A deeper blind spot is the
...
approaches~\cite{Sequeda2019pay} and ontology modularisation~\cite{LeClair2022modularization}
...
affordances~\cite{gibson} and Norman's design affordances~\cite{norman}, has been extensively applied in HCI and design, but has barely
...
\add{Recently, Celino \cite{celino2026affordances} argued the utility of affordance-based characterisations within the information-seeking domain, and this notion of affordance is deliberately limited to information-seeking tasks. The ontological continuum extends the same insight to knowledge engineering operations, where interoperability is constituted not by the ability to answer a query but by the operations that the formalisation, the conceptual model, and the upper-level commitment make available to a community. In our paper, information seeking is one affordance class the continuum supports; it is not the framework's primary motivation or scope.}
