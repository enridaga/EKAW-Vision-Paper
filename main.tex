...
% TODO: Revise pragmatic affordances in the tables as follows: (a) limit to language-level affordances (SPARQL, SHACL, OWL/DL profiles); (b) remove fine-grained SPARQL-over-specific-vocabulary mappings, which conflate high-level affordances with derived ones; (c) add OWL DL profile support where applicable. This revision is pending the expanded KG population (see below).
% TODO: Expand the KG population to include additional examples that share pragmatic properties with existing ones, so that the FCA lattice exhibits meaningful overlap. Current population is one-per-method, which produces a sparse lattice.
